...
โมดูลนี้จึงรวม 3 เรื่องนั้นเข้ากับส่วนต่าง % เป็นคะแนนเดียว 0-100:
    ...
    base_severity = deviation (0-80) + exposure (0-10) + direction (0-10)
    confidence    = f(จำนวนสมาชิกกลุ่ม) x f(CV = std/mean ของกลุ่ม) x f(metric ที่ใช้เทียบ)  [พื้น 0.55]

ตัวคูณ 3 ตัวนั้นตั้งใจให้ "ลดทอน" คะแนนเท่านั้น ไม่ใช่ตัวตัดสินหลัก — เดิมตั้งไว้แรงเกินไป (ต่ำสุดถึง 0.55 x
0.65 x 0.85 = 0.30) จนใบที่สำแดงต่ำกว่าราคากลางมากและส่วนต่างเป็นเงินหลายล้านบาท ยังตกลงมาเป็นเขียวได้เพียง
เพราะกลุ่มอ้างอิงมีสมาชิกไม่ถึง 10 รายการหรือราคาในกลุ่มกระจายกว้าง ซึ่งกลับหัวกลับหางกับเจตนาของคะแนน (ความ
เชื่อมั่นของ "ฐานที่เอาไปเทียบ" ไม่ควรกลบความรุนแรงของสิ่งที่ตรวจพบจนเปลี่ยนสี) จึงบีบช่วงของทั้ง 3 ตัวเข้าหา
1.00 ให้ penalty รวมเหลือประมาณครึ่งเดียวของเดิม — ลำดับ/ทิศทางของสัญญาณยังเหมือนเดิมทุกตัว (กลุ่มเล็ก/กระจาย
กว้าง/เทียบด้วยมูลค่ารวม ยังได้คะแนนน้อยกว่าเสมอ) แค่ไม่ข้ามระดับสีเพราะเหตุผลนี้อย่างเดียวอีกต่อไป
...
#
# เดิมกระจายเป็น 45/35/20 ซึ่งทำให้ "ขนาดของใบขน" (exposure) มีน้ำหนักเกือบเท่าความรุนแรงของการสำแดงราคาผิด
# เอง ผลคือใบที่สำแดงต่ำกว่าราคากลางลึกมากแต่เป็นของชิ้นเล็ก แตะระดับสีสูงไม่ได้เลยไม่ว่าจะต่ำผิดปกติแค่ไหน
# ในขณะที่ใบใหญ่ได้แต้มฟรีจากขนาดตัวเอง — ย้ายน้ำหนักส่วนใหญ่มาที่ตัวส่วนต่าง % จากราคากลางแทน (ตัวชี้ว่า
# "สำแดงผิดปกติจริงไหม") เหลือ exposure/direction ไว้เป็นตัวจัดลำดับระหว่างใบที่ผิดปกติเท่ากัน ไม่ใช่ตัวตัดสิน
W_DEVIATION = 80
W_EXPOSURE = 10
W_DIRECTION = 10
...
# ถ้าสเกลเชิงเส้น ใบใหญ่ใบเดียวจะกลืนคะแนนของทุกใบที่เหลือจนแยกความต่างไม่ออก — ช่วงนี้ไม่ต้องแคบลงตาม
# W_EXPOSURE ที่ลดเหลือ 10 เพราะหน้าที่ของมันเปลี่ยนไปแล้ว: จากตัวผลักข้ามระดับสี เหลือแค่ตัวเรียงลำดับใบที่
# ส่วนต่าง % เท่ากันว่าใบไหนกระทบเงินมากกว่า (10 แต้มเต็มพอสำหรับเรียงลำดับ ไม่พอสำหรับเปลี่ยนสีเองลำพัง)
...
# ตัวคูณความเชื่อมั่นมีพื้นไม่ใช่ 0 — ฐานอ้างอิงอ่อนแค่ไหนก็ยังไม่ควรกดคะแนนลงเหลือ 0 เพราะส่วนต่างที่ตรวจพบ
# ยังเป็นข้อเท็จจริงที่เกิดขึ้นจริง แค่หลักฐานสนับสนุนอ่อนลง — เป็น backstop เผื่อมีการปรับตาราง 3 ฟังก์ชัน
# ข้างล่างให้ต่ำลงในอนาคต ค่าตารางชุดปัจจุบันคูณกันแล้วต่ำสุดได้ 0.80 x 0.85 x 0.93 = 0.63 ยังไม่แตะพื้นนี้
...
def _count_factor(n: int | None) -> tuple[float, str]:
    ...
    if n is None:
        return 0.85, "ไม่มีข้อมูลจำนวนสมาชิกกลุ่ม (โมเดลเทรนก่อนเพิ่มการเก็บค่านี้)"
    if n >= 30:
        return 1.00, f"กลุ่มอ้างอิงมีสมาชิก {n:,} รายการ — ค่าเฉลี่ยนิ่งพอ"
    if n >= 10:
        return 0.95, f"กลุ่มอ้างอิงมีสมาชิก {n:,} รายการ — พอใช้ได้"
    if n >= 5:
        return 0.88, f"กลุ่มอ้างอิงมีสมาชิกแค่ {n:,} รายการ — ค่าเฉลี่ยยังไม่นิ่ง"
    return 0.80, f"กลุ่มอ้างอิงมีสมาชิกแค่ {n:,} รายการ — ฐานอ้างอิงอ่อนมาก"
...
def _cv_factor(cv: float | None) -> tuple[float, str]:
    ...
    if cv is None:
        return 0.95, "ไม่มีข้อมูลการกระจายราคาของกลุ่ม (โมเดลเทรนก่อนเพิ่มการเก็บค่านี้)"
    if cv <= 0.30:
        return 1.00, f"ราคาในกลุ่มเกาะกันแน่น (CV {cv:.2f})"
    if cv <= 0.60:
        return 0.95, f"ราคาในกลุ่มกระจายปานกลาง (CV {cv:.2f})"
    if cv <= 1.00:
        return 0.90, f"ราคาในกลุ่มกระจายค่อนข้างกว้าง (CV {cv:.2f})"
    return 0.85, f"ราคาในกลุ่มกระจายกว้างมาก (CV {cv:.2f}) — ค่าเฉลี่ยแทนกลุ่มได้ไม่ดี"
...
def _metric_factor(metric: str) -> tuple[float, str]:
    ...
    return 0.93, "เทียบด้วยมูลค่ารวม (CIF) เพราะไม่มีน้ำหนักใช้ได้ — ยังปนผลของปริมาณที่สั่ง"
...
def assess(row: Mapping) -> dict:
    ...
    แถวที่โมเดลตัดสินว่า normal จะได้คะแนนจากปัจจัยส่วนต่างอย่างเดียว (ไม่คิด exposure/direction) โดยตั้งใจ —
    ราคาที่ยังอยู่ในกรอบปกติของกลุ่มไม่ควรกลายเป็นเหลืองเพียงเพราะเป็นใบขนมูลค่าสูง (มิฉะนั้นใบใหญ่ทุกใบจะติด
    เหลืองยกแผง กลบใบที่ผิดปกติจริง)

    normal จึงยังเป็นเขียวตลอด แต่ตอนนี้มาจากคนละเหตุผลกับเดิม (เดิมเพราะ W_DEVIATION = 45 < TIER_YELLOW_MIN
    ตรงๆ ตอนนี้ W_DEVIATION = 80 ซึ่งเกินขอบเหลืองไปแล้ว) — normal นิยามว่าอยู่ในกรอบ mean x (1 +- 0.5) แปลว่า
    deviation_pct <= 50 เสมอ คะแนนสูงสุดที่เป็นไปได้จึงเป็น 80 x (50 / DEV_CAP_BELOW) = 40 < TIER_YELLOW_MIN
    = 46 (ฝั่งสูงกว่าราคากลางยิ่งน้อยกว่าอีกเพราะ DEV_CAP_ABOVE กว้างกว่า)

    ข้อนี้ผูกกับ alert_ratio = 0.5 ที่ใช้ตอนเทรน (ดู train.py --alert-ratio, pipeline.PREDICT_ALERT_RATIO)
    ไม่ใช่ค่าคงที่ในไฟล์นี้อย่างเดียวอีกต่อไป — ถ้าวันไหนขยาย alert_ratio เกิน ~0.575 (= 46/80) แถว normal จะ
    เริ่มติดเหลืองได้เอง ต้องลด W_DEVIATION หรือแยกเพดานของแถว normal ออกมาต่างหาก"""
