...
Flag รายการที่ CIFVALTHB ต่ำ/สูงกว่าช่วง ±ratio ของค่าเฉลี่ยกลุ่มย่อย -> Alert Undervalue/Overvalue
        │
        ▼
ให้คะแนนความเสี่ยง 0-100 ครอบผล flag อีกชั้น (ส่วนต่าง % + ส่วนต่างเป็นเงินบาท + ทิศทาง แล้วคูณความ
เชื่อมั่นของฐานราคาอ้างอิง) -> ระดับสีที่ใช้จัดลำดับตรวจ (ดูหัวข้อ "คะแนนความเสี่ยง (Risk Score)")
```
...
## คะแนนความเสี่ยง (Risk Score) — ชั้นที่ครอบผล Undervalue/Overvalue

ผลดิบจากโมเดลตอบได้แค่ "ทิศทาง" (undervalue/overvalue/normal) กับ "ส่วนต่าง % จากราคากลาง" ซึ่งยังไม่พอ
จัดลำดับว่าเจ้าหน้าที่ควรหยิบใบไหนตรวจก่อน — สำแดงต่ำกว่าราคากลาง 90% ของสินค้ามูลค่า 3,000 บาท ไม่สำคัญ
เท่าสำแดงต่ำกว่า 60% ของสินค้ามูลค่า 8 ล้านบาท และการหลุด threshold ของกลุ่มที่ตอนเทรนมีสมาชิกแค่ 3 รายการ
ราคากระจายกว้าง ก็เป็นหลักฐานที่อ่อนกว่ากลุ่มที่ราคาเกาะกันแน่น `webapp/risk.py` จึงรวมเรื่องพวกนี้เป็น
คะแนนเดียว 0-100 ต่อรายการ:

```
risk_score = base_severity x confidence

base_severity = deviation (0-80) + exposure (0-10) + direction (0-10)
  deviation  ส่วนต่าง % จากราคากลางกลุ่ม เต็มคะแนนที่ 100% (ฝั่งต่ำกว่า) / 200% (ฝั่งสูงกว่า)
  exposure   |ราคาที่ควรเป็น - ที่สำแดง| เป็นบาท ให้คะแนนแบบ log (1,000 บาท = 0, 10 ล้านบาท = เต็ม)
  direction  undervalue เต็ม 10 (รัฐสูญรายได้ภาษีตรงๆ) / overvalue 5 / normal 0

confidence = f(จำนวนสมาชิกกลุ่ม) x f(CV = std/mean ของกลุ่ม) x f(metric ที่ใช้เทียบ)   [พื้น 0.55]
```

น้ำหนักส่วนใหญ่อยู่ที่ **ส่วนต่าง % จากราคากลาง** โดยเจตนา — exposure/direction เหลือไว้เป็นตัวจัดลำดับระหว่าง
ใบที่สำแดงผิดปกติเท่ากันว่าใบไหนควรหยิบตรวจก่อน ไม่ใช่ตัวผลักข้ามระดับสีเอง (เดิม 45/35/20 ทำให้ "ขนาดของใบขน"
มีน้ำหนักเกือบเท่าความรุนแรงของการสำแดงผิดเอง ใบเล็กที่ต่ำกว่าราคากลางลึกมากจึงแตะระดับสีสูงไม่ได้เลย)

ระดับสี/KPI/ตัวกรองบนหน้าเว็บ (ขา test) มาจากคะแนนนี้: **0-45 เขียว / 46-75 เหลือง / 76-100 แดง** ส่วนแถวที่
ไม่มีฐานราคาอ้างอิงเลยยังเป็น bucket ของตัวเองเหมือนเดิม (**No Model** / **New Cluster**) และไม่มีคะแนน —
"ประเมินไม่ได้" ไม่เท่ากับ "ปลอดภัย"

หน้ารายละเอียด (drawer) มี 2 การ์ดอธิบายที่มาของคะแนน:

- **⚙ AI/ML — วิธีที่ใช้ประเมิน**: เทคนิคที่ใช้ (ไฮไลต์เฉพาะตัวที่พบสัญญาณกับใบนั้นจริง) + ภารกิจที่สแกน
  5 ด้าน พร้อมสถานะ **พบสัญญาณ / ปกติ / ยังไม่ประเมิน** — ตอนนี้ทำงานจริงด้านเดียวคือตรวจราคาผิดปกติ
  (คะแนนความเสี่ยงทั้งหมดมาจากด้านนี้) อีก 4 ด้าน (สำแดงพิกัดผิด, สวมถิ่นกำเนิด/สวมสิทธิ, ชี้เป้าความเสี่ยงรวม,
  ตรวจสอดคล้องเอกสาร) เป็นโครงรอต่อข้อมูล/โมเดลจริง จึงขึ้น "ยังไม่ประเมิน" — ตั้งใจแยกจาก "ปกติ" ไม่ให้เข้าใจ
  ผิดว่าด้านนั้นผ่านการตรวจมาแล้ว (ดู `_tasks` ใน [webapp/risk.py](webapp/risk.py))
- **ผลรวมคะแนน ที่ AI/ML ประเมิน**: ตารางแท่งแจกแจงทุกปัจจัย (แต้ม/น้ำหนักเต็ม) → คะแนนดิบรวม → ตัวคูณ
  ความเชื่อมั่น 3 ตัว → คะแนนสุทธิ ตรวจย้อนที่มาของคะแนนได้ทุกบรรทัด ไม่ใช่เลขลอย ๆ จากโมเดล

รายการที่โมเดล flag เป็น undervalue/overvalue จึงเป็นสีเขียวได้ ถ้าส่วนต่างคิดเป็นเงินน้อยและ/หรือฐานราคา
อ้างอิงของกลุ่มยังอ่อน (ทิศทางที่ตรวจพบยังแสดงอยู่ในหน้ารายละเอียดครบเหมือนเดิม) ปรับน้ำหนัก/เพดาน/เกณฑ์สี
ได้ที่ค่าคงที่หัวไฟล์ `webapp/risk.py` — **ทุกครั้งที่แก้ ต้อง bump `RISK_VERSION`** แล้วคะแนนของแถวที่สะสมไว้
จะถูกคิดใหม่เองอัตโนมัติรอบ sync ถัดไป (คิดใหม่เฉย ๆ ในหน่วยความจำ ไม่ได้ predict/embed ซ้ำ)
